...
public class ActivityComment extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        image_sent.setOnClickListener(view -> {
            if (myApplication.getIsLogin()) {
                if (edt_comment.length() == 0) {
                    ...
                    JsonObject jsObj1 = (JsonObject) new Gson().toJsonTree(new API());
                    jsObj1.addProperty("method_name", "video_comment");
                    jsObj1.addProperty("comment_text", edt_comment.getText().toString());
                    jsObj1.addProperty("user_name", myApplication.getUserName());
                    jsObj1.addProperty("user_id", myApplication.getUserId());
                    jsObj1.addProperty("post_id", Constant.LATEST_CMT_IDD);
                    if (JsonUtils.isNetworkAvailable(ActivityComment.this)) {
                        new MyTaskComment(API.toBase64(jsObj1.toString())).execute(Constant.API_URL);
                    }
